    ...
    <section class="section">
        <div class="container py-4">
            ...
            <div class="row">
                ...
                <div class="col-md-9">
                      <input type="hidden" id="search-query-term" value="{{ $searchQuery ?? '' }}">
                     <!--- new filter section --->
...
<script>
    ...
    $(document).ready(function() {
        ...
        function setFiltersFromUrl() {
            ...
            $('#price-range-display').text(`Price: ৳ ${$('#min-price-slider').val()} - ৳ ${$('#max-price-slider').val()}`);
            ...
            if (params.has('sizes[]')) {
                params.getAll('sizes[]').forEach(size => $(`.size-filter[value="${size}"]`).prop('checked', true));
            }
         ...
         * THIS FUNCTION IS NOW UPDATED FOR SEARCH.
         */
        function getFilters() {
            const selectedSizes = $('.size-filter:checked').map(function() { return $(this).val(); }).get();
            let filters = {
                // NEW: Read the search query from the hidden input
                query: $('#search-query-term').val(),
        ...
        function loadProducts(reset = false) {
            ...
            // This logic correctly determines which AJAX URL to use
        ...
        function updateActiveFiltersDisplay() {
            ...
            $('.main-category-filter.active, .subcategory-filter.active, .animation-category-filter.active').each(function() {
                ...
                const type = $(this).hasClass('main-category-filter') ? 'category' : $(this).hasClass('subcategory-filter') ? 'subcategory' : 'animation';
